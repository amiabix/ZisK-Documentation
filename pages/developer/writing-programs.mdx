...
Let's show these changes using the example program from the [Quickstart](/getting-started/quickstart) section.
...
This command compiles the program using the `riscv64ima-zisk-zkvm-elf` target. The resulting `sha_hasher` ELF file (without extension) is generated in the `./target/riscv64ima-zisk-zkvm-elf/debug` directory.
...
- [GPU Proof Generation](/optimization/gpu-proof-generation) – Accelerate proof generation using NVIDIA GPUs.
- [Concurrent Proof Generation](/optimization/concurrent-proof-generation) – Generate proofs in parallel using multiple processes.
- [Distributed Proof Generation](/distributed/introduction) – Scale proof generation across multiple machines for large workloads.
