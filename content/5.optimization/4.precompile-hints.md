...
![Hints pipeline](/images/hints-pipeline.svg)
...
The hints protocol supports chunking for individual hints that exceed the transport's message size limit (currently 128 KB). Each message in the stream contains either a single complete hint or one chunk of a larger hint.
...
Custom hint codes can technically use any value not occupied by control or built-in codes. By convention, codes `0xA000`-`0xFFFF` are recommended for custom use to avoid future conflicts as new built-in types are added. The processor does not enforce a range restriction.
::
...
| `0x0002` | `CTRL_CANCEL` | **[Reserved for future use]** Cancels the current stream. |
| `0x0003` | `CTRL_ERROR` | **[Reserved for future use]** External error signal. |
...
Four CLI commands (`execute`, `prove`, `verify-constraints`, `stats`) support the hints stream system by providing a URI via the `--hints` option. The URI determines the input stream source for hints, which can be a file, Unix socket, QUIC stream, or other custom transport.
...
::callout{icon="i-heroicons-information-circle"}
Only ASM mode supports hints. The emulator mode does not use the hints pipeline.
::
...
![Hints distributed execution](/images/hints-distributed.svg)
...
When starting a worker, if the `--hints` option is provided, the worker prepares to receive hints from the coordinator. When launching a proof generation job where hints will be provided, the workers must be started to receive and process hints.
...
#### Coordinator Hints Streaming Mode
...
#### Worker Hints non-Streaming Mode
...
If a hints file was generated, it can be consumed using the `--hints` flag in the `cargo-zisk` commands that support hints (as explained in [Hints in CLI Execution](#hints-in-cli-execution)).
